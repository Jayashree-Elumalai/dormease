...
class _RegisterScreenState extends State<RegisterScreen> {
  final _auth = FirebaseAuth.instance;//for auth
  final _firestore = FirebaseFirestore.instance;//for db
  ...
  //state variables-track ui state
  bool _loading = false;//loading
  bool _obscurePassword = true;//toggle pass (hide)
  bool _obscureConfirm = true;//toggle confirm pass (hide)
  String _error = '';

  //validation error (true= got error0
  bool _nameError = false;
  bool _emailError = false;
  bool _emailTakenError = false;//email alrd taken
  bool _studentIdError = false;
  bool _studentIdTakenError = false;// student id alrd taken
  ...
  void _validateFields() {
    setState(() {
      ...
      // student ID: 3-20 alphanumeric characters, hyphens, parentheses
      _studentIdError = _studentIdCtrl.text.trim().isEmpty ||
          !RegExp(r'^[A-Za-z0-9\-\(\)]{3,20}$').hasMatch(_studentIdCtrl.text.trim());

      _passwordError = _passwordCtrl.text.length < 8;// pass: min 8 chars
      _confirmError = _confirmCtrl.text != _passwordCtrl.text;// must match pass
      _emergencyError =// 8-15 digits
      !RegExp(r'^\d{8,15}$').hasMatch(_emergencyCtrl.text.trim());
      // room: 3-20 alphanum chars, hyphens, spaces
  ...
  //register new student acc
  Future<void> _register() async {
    _validateFields();// validate fields
    setState(() {// reset alrd taken errors
    ...
    try {
      ...
      final userDoc = {
        ...
        'nameLower': _nameCtrl.text.trim().toLowerCase(),//for search
        'email': _emailCtrl.text.trim(),
        ...
        'rejectionReason': null, // no rejection yet
    ...
    } on FirebaseAuthException catch (e) { //handle errors
      if (e.code == 'email-already-in-use') {
        setState(() {
          _emailTakenError = true;//email taken
        });
  ...
  // build reusable text field widget ui
  Widget _buildTextField({
    ...
    List<TextInputFormatter>? inputFormatters,//restrict input
    bool obscure = false,//hide text for pass
  }) {
    return TextField(
        ...
        child: SingleChildScrollView(//scrollable
          ...
          child: Column(
            ...
            children: [
              ...
              if (_nameError)//show error if invalid
                Padding(
              ...
              if (_emailError || _emailTakenError)//show error if invalid /alrd taken
                Padding(
                  padding: const EdgeInsets.only(top: 6),
                  child: Text(
                    _emailTakenError
              ...
              _buildTextField(
                ...
                inputFormatters: [
                  // only allow alphanumeric, hyphens, parentheses
                  FilteringTextInputFormatter.allow(RegExp(r'[A-Za-z0-9\-\(\)]')),
                  LengthLimitingTextInputFormatter(20),//max 20 chars
                ],
              ),
              //show error if invalid /alrd taken
              ...
              _buildTextField(
                ...
                obscure: _obscurePassword,// hide pass
              ...
              if (_passwordError)//show error if pass too short
                Padding(
                    padding: const EdgeInsets.only(top: 6),
              ...
              if (_confirmError)//show error if passwords dont match
                Padding(
                    padding: const EdgeInsets.only(top: 6),
              ...
              _buildTextField(
                ...
                inputFormatters: [
                  FilteringTextInputFormatter.digitsOnly,//only nums
                  LengthLimitingTextInputFormatter(15),
                ],
              ),
              if (_emergencyError)//show error if invalid
                Padding(
                    padding: const EdgeInsets.only(top: 6),
              ...
              _buildTextField(
                ...
                inputFormatters: [
                  FilteringTextInputFormatter.allow(RegExp(r'[a-zA-Z0-9]')),//alphanum only
                  LengthLimitingTextInputFormatter(20),
                ],//max 20 chars
              ),
              if (_blockError)//show error if invalid
                Padding(
                  padding: const EdgeInsets.only(top: 6),
              ...
              _buildTextField(
                ...
                inputFormatters: [
                  FilteringTextInputFormatter.allow(RegExp(r'[A-Za-z0-9\-\s]')),//alphanum chars, hyphens, spaces
                  LengthLimitingTextInputFormatter(20),
                ],
              ),
              if (_roomError)//show error if invalid
                Padding(
                    padding: const EdgeInsets.only(top: 6),
              ...
              if (_error.isNotEmpty)// show general error message
                Text(_error,
                    style:
